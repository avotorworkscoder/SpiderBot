...
// 30ms = servo controller always has a near-future target to track.
const uint16_t SMOOTH_T = 30;
...
void stopSmooth() {
  targetStrength = 0.0f;
  // gaitRunning stays true until realStrength eases to ~0 (handled in tick)
}
...
void smoothGaitTick() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    // Arduino Z convention: more negative = up → subtract offLift
