 ...
 * Updates: Connected to Serial Bluetooth Terminal App.
 *          Added tripodStep, rotateStep, standHome funcitons.
 *
...
Leg legs[6] = {
    ...
    {3, 10, 0, -8},   // Mid-Left
    {29, 10, -8, -8}, // Rear-Left
    {6, 10, -8, -8},  // Front-Right
    {9, 10, 0, -8},   // Mid-Right
    {26, 10, 8, -8},  // Rear-Right
};
...
void btCallback(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
  if (event == ESP_SPP_SRV_OPEN_EVT)
  {
    ...
    SerialBT.println("SpiderBot V2.1 Ready!");
...
void tripodStep(float strideY)
{
  ...
  for (int i = 0; i < 3; i++)
    moveFootXYZ(legs[A[i]].id, legs[A[i]].restX, legs[A[i]].restY + strideY, legs[A[i]].restZ - 3.0f, moveTime);
  for (int i = 0; i < 3; i++)
    moveFootXYZ(legs[B[i]].id, legs[B[i]].restX, legs[B[i]].restY - strideY, legs[B[i]].restZ, moveTime);
  delay(moveTime + 20);

  for (int i = 0; i < 3; i++) // A plants
    moveFootXYZ(legs[A[i]].id, legs[A[i]].restX, legs[A[i]].restY + strideY, legs[A[i]].restZ, moveTime / 2);
  delay(moveTime / 2 + 20);

  // Phase 2: B lifts & swings, A pushes
  for (int i = 0; i < 3; i++)
    moveFootXYZ(legs[B[i]].id, legs[B[i]].restX, legs[B[i]].restY + strideY, legs[B[i]].restZ - 3.0f, moveTime);
  for (int i = 0; i < 3; i++)
    moveFootXYZ(legs[A[i]].id, legs[A[i]].restX, legs[A[i]].restY - strideY, legs[A[i]].restZ, moveTime);
  delay(moveTime + 20);

  for (int i = 0; i < 3; i++) // B plants
    moveFootXYZ(legs[B[i]].id, legs[B[i]].restX, legs[B[i]].restY + strideY, legs[B[i]].restZ, moveTime / 2);
  delay(moveTime / 2 + 20);
}
...
void rotateStep(float yawOffset)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    float sign = (legs[A[i]].restY > 0) ? 1.0f : -1.0f;
    moveFootXYZ(legs[A[i]].id, legs[A[i]].restX + sign * yawOffset, legs[A[i]].restY, legs[A[i]].restZ - 3.0f, moveTime);
  }
  for (int i = 0; i < 3; i++)
  {
    float sign = (legs[B[i]].restY > 0) ? -1.0f : 1.0f;
    moveFootXYZ(legs[B[i]].id, legs[B[i]].restX + sign * yawOffset, legs[B[i]].restY, legs[B[i]].restZ, moveTime);
  }
  delay(moveTime + 20);
  for (int i = 0; i < 3; i++)
    moveFootXYZ(legs[A[i]].id, legs[A[i]].restX, legs[A[i]].restY, legs[A[i]].restZ, moveTime / 2);
  delay(moveTime / 2 + 20);

  for (int i = 0; i < 3; i++)
  {
    float sign = (legs[B[i]].restY > 0) ? 1.0f : -1.0f;
    moveFootXYZ(legs[B[i]].id, legs[B[i]].restX + sign * yawOffset, legs[B[i]].restY, legs[B[i]].restZ - 3.0f, moveTime);
  }
  for (int i = 0; i < 3; i++)
  {
    float sign = (legs[A[i]].restY > 0) ? -1.0f : 1.0f;
    moveFootXYZ(legs[A[i]].id, legs[A[i]].restX + sign * yawOffset, legs[A[i]].restY, legs[A[i]].restZ, moveTime);
  }
  delay(moveTime + 20);
  for (int i = 0; i < 3; i++)
    moveFootXYZ(legs[B[i]].id, legs[B[i]].restX, legs[B[i]].restY, legs[B[i]].restZ, moveTime / 2);
  delay(moveTime / 2 + 20);
}
...
void handleCommand(char cmd)
{
  ...
  case '-':
    if (moveTime < SPEED_MAX)
      moveTime += SPEED_STEP;
    SerialBT.print(">> Speed dn → ");
    SerialBT.print(moveTime);
    SerialBT.println("ms");
    break;

  case '?':
    SerialBT.println("─── SpiderBot V2.1 ───");
...
void setup()
{
  ...
  Serial.println("SpiderBot V2.1 — Bluetooth started");
