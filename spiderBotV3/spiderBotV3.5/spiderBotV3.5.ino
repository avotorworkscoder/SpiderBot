/*===============================================================
 * SpiderBot V3.5 — PHASE 3 INTEGRATION (BÉZIER CONTINUITY)
 * Author  : Amit Parihar
 ...
 *   5.  Gait Engine         — Parametric Continuous Engine
 *       • Hardware offloading eliminated. Streaming 30ms slices!
 ...
 *  13.  setup() + loop()
 *
 * ── PHASE 3 INTEGRATION (ORGANIC BÉZIER CURVES) ──────────────
 *   + Parametric Engine:
 *       • The Gait Engine no longer issues "Swing" and "Plant" as
 *         separate linear jumps.
 *       • Evaluates mathematical progress `t` (0.0 to 1.0) and streams
 *         micro-coordinates at 30ms slices directly to IK logic.
 *   + Cubic Bézier Z-Axis Arcs:
 *       • Replaced square robotic lifting with a smooth, S-shaped 
 *         4-point mathematical curve. Limits jitter and collision.
 *
 * ── PHASE 2 INTEGRATION (3D BODY KINEMATICS) ─────────────────
 *   + Local/Global Coordinate System Transformation:
 *       • True 3D Rotational Matrix applied to body tilt inside `moveFootXYZ`.
 *       • Gait algorithms are now completely mathematically isolated from 
 *         terrain angle logic.
 *
 * ── BUTTON MAP ───────────────────────────────────────────────
...
void moveFootXYZ(uint8_t id, float x, float y, float z, uint16_t t, bool perpTibia = false) {
  ...
  if (imuReady && isBotOn && imuActive) {
    // 2. Body tilts (Roll around Y axis, Pitch around X axis)
    // Roll is inverted to match the IMU's orientation and prevent positive feedback
    ...
    // 3. New Global position of the Hip attached to the tilted body
    float rHipX = leg.hipX * cr + 0.0f * sr;
    float rHipY = leg.hipY * cp + leg.hipX * sp * sr;
    float rHipZ = leg.hipY * sp - leg.hipX * cp * sr;
    ...
    // Undoing pitch (Rx_inv) then roll (Ry_inv)
    float iy = vecY * cp + vecZ * sp;
    float iz = -vecY * sp + vecZ * cp;
    
    float localX = vecX * cr - iz * sr;
    float localY = iy;
    float localZ = vecX * sr + iz * cr;
    ...
    // Basic terrain adjustment when IMU is off
    z = footWorldZ; 
  }
...
// 8-state quarter-time keyframe machine.
// Each state fires ONE servo command covering 1/4 of the Bézier arc,
// with quarterTime = moveTime/4 duration each. This produces natural,
// gradual acceleration rather than two abrupt large jumps.
//
// Bézier Z heights at each keyframe (LIFT_H=2.2, pt1=pt2=3.52):
//   Q1 (t=0.25): Z = 1.98 cm  — gentle initial lift
//   Q2 (t=0.50): Z = 2.64 cm  — reaching peak
//   Q3 (t=0.75): Z = 1.98 cm  — gentle descent
//   Q4 (t=1.00): Z = 0.00 cm  — soft landing
enum GaitPhase {
  IDLE,
  A_Q1, A_Q2, A_Q3, A_Q4,   // Group A swings, B drags
  B_Q1, B_Q2, B_Q3, B_Q4,   // Group B swings, A drags
  RECENTER, WAVE_STEP, WAVE_PLANT
};
...
// Streamed parametric trajectory replaces fixed linear jump
void tickContinuousTripod(float t, bool groupASwings, uint16_t timeSlice) {
  GaitDef& g = GAIT_TABLE[activeGait];
  const int* swingGrp = groupASwings ? GRP_A : GRP_B;
  const int* dragGrp  = groupASwings ? GRP_B : GRP_A;

  // Bézier Z-axis control points for an organic arc
  // Starts flat, lifts abruptly, lingers in the air, then drops onto target softly
  float pt0 = 0.0f;           
  float pt1 = LIFT_H * 1.6f;  
  float pt2 = LIFT_H * 1.6f;  
  float pt3 = 0.0f;           
  float bezierZ = bezierCubic(t, pt0, pt1, pt2, pt3);

  // Group 1: Swinging through the air
  for (int i = 0; i < 3; i++) {
    int   idx    = swingGrp[i];
    Leg&  leg    = crabMode ? crabLegs[idx] : legs[idx];
    ...
    float startX = leg.restX + (g.dragX(idx) - leg.restX) * scaleX;
    float endX   = leg.restX + (g.swingX(idx) - leg.restX) * scaleX;
    float curX   = startX + (endX - startX) * t;

    float startY = leg.walkY + (g.dragY(idx) - leg.walkY) * scaleY;
    float endY   = leg.walkY + (g.swingY(idx) - leg.walkY) * scaleY;
    float curY   = startY + (endY - startY) * t;

    moveFootXYZ(leg.id, curX, curY, leg.restZ + bezierZ, timeSlice);
  }

  // Group 2: Dragging on the floor
  for (int i = 0; i < 3; i++) {
    int   idx    = dragGrp[i];
    Leg&  leg    = crabMode ? crabLegs[idx] : legs[idx];
    float scaleY = g.indepXY ? jFwd : constrain(jFwd + (jTurn * leg.coxaDir), -1.0f, 1.0f);
    ...
    float startX = leg.restX + (g.swingX(idx) - leg.restX) * scaleX;
    float endX   = leg.restX + (g.dragX(idx) - leg.restX) * scaleX;
    float curX   = startX + (endX - startX) * t;

    float startY = leg.walkY + (g.swingY(idx) - leg.walkY) * scaleY;
    float endY   = leg.walkY + (g.dragY(idx) - leg.walkY) * scaleY;
    float curY   = startY + (endY - startY) * t;

    moveFootXYZ(leg.id, curX, curY, leg.restZ, timeSlice);
  }
}
...
void wavePlant(uint8_t phaseIdx) {
  ...
  moveFootXYZ(leg.id, leg.restX, plantY, leg.restZ, plantTime);
}
...
void startGait(bool continuous) {
  ...
  if (activeGait == 3) {
    ...
    // Fire first keyframe immediately: Group A rises to 1st quarter of arc.
    uint16_t quarterTime = moveTime / 4;
    tickContinuousTripod(0.25f, true, quarterTime);
    gaitPhase  = A_Q1;
  }
  phaseStart = millis();
}
...
void gaitTick() {
  ...
  uint16_t      qTime    = moveTime / 4; // Each keyframe lasts one quarter of moveTime
  uint16_t      sTime    = qTime > 10 ? qTime - 5 : qTime; // Servo finishes 5ms early → no micro-pause at boundary

  switch (gaitPhase) {

    // ── Group A swings, Group B drags (4 smooth keyframes) ───────
    case A_Q1: // Fired on entry at t=0.25 (partial lift)
      if (elapsed >= qTime) {
        tickContinuousTripod(0.50f, true, sTime); // Reach peak height
        gaitPhase = A_Q2; phaseStart = now;
      }
      break;

    case A_Q2: // At t=0.5 (peak)
      if (elapsed >= qTime) {
        tickContinuousTripod(0.75f, true, sTime); // Begin gentle descent
        gaitPhase = A_Q3; phaseStart = now;
      }
      break;

    case A_Q3: // At t=0.75 (partial descent)
      if (elapsed >= qTime) {
        tickContinuousTripod(1.00f, true, sTime); // Soft landing
        gaitPhase = A_Q4; phaseStart = now;
      }
      break;

    case A_Q4: // At t=1.0 (landed) — transition to Group B swing
      if (elapsed >= qTime) {
        tickContinuousTripod(0.25f, false, sTime); // Group B begins gentle lift
        gaitPhase = B_Q1; phaseStart = now;
      }
      break;

    // ── Group B swings, Group A drags (4 smooth keyframes) ───────
    case B_Q1:
      if (elapsed >= qTime) {
        tickContinuousTripod(0.50f, false, sTime);
        gaitPhase = B_Q2; phaseStart = now;
      }
      break;

    case B_Q2:
      if (elapsed >= qTime) {
        tickContinuousTripod(0.75f, false, sTime);
        gaitPhase = B_Q3; phaseStart = now;
      }
      break;

    case B_Q3:
      if (elapsed >= qTime) {
        tickContinuousTripod(1.00f, false, sTime);
        gaitPhase = B_Q4; phaseStart = now;
      }
      break;

    case B_Q4:
      if (elapsed >= qTime) {
        if (!looping && fabsf(jFwd) < 0.05f && fabsf(jTurn) < 0.05f) {
          gaitPhase = IDLE;
          recenterLegs();
        } else {
          tickContinuousTripod(0.25f, true, sTime); // Restart: Group A gentle lift
          gaitPhase = A_Q1; phaseStart = now;
        }
      }
      break;

    case RECENTER:
      if (elapsed >= (unsigned long)(moveTime / 2)) {
        recenterLegs();
        gaitPhase = IDLE;
      }
      break;
    ...
    case WAVE_PLANT:
      if (elapsed >= (unsigned long)moveTime) {
...
void updateLevelling() {
  ...
  for (int i = 0; i < 6; i++) {
    Leg& leg = crabMode ? crabLegs[i] : legs[i];
    moveFootXYZ(leg.id, leg.restX, leg.walkY, leg.restZ, LEVEL_INTERVAL_MS, true);
  }
