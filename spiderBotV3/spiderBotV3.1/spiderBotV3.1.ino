 ...
 *  10. setup() + loop()
 *
 *  [RETROACTIVE FIX]: inverse kinematics, 90-degree femur bugfix, 
 *  and perpTibia height adjustment.
 *
 * ── WHAT'S NEW IN V3.0 vs V2.8 ──────────────────────────────
 *   - PS2 DualShock controller (PS2X_lib) is the primary input.
 *   - BT Serial remains active as a debug / override channel.
 *   - Left stick  → jFwd (Y) + jTurn (X) — differential drive
 *   - Right stick → jStrafe (X-axis, used in crab/diagonal mode)
 *   - D-pad       → discrete movement commands (same as BT F/B/L/R)
 *   - △ / ×       → height up / down
 *   - ○           → sitting position
 *   - □           → sleeping position
 *   - SELECT      → stop / stand still
 *   - START       → cycle through gaits  g0→g1→g2→g3→g0
 *   - L1          → rotate left  (held)
 *   - R1          → rotate right (held)
 *   - L2          → speed down
 *   - R2          → speed up
...
void setup() {
  ...
  Serial.println("SpiderBot V3.0 — Bluetooth: SpiderBot");
