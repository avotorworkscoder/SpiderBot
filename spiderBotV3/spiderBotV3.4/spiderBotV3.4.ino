/*===============================================================
 * SpiderBot V3.4 — PHASE 2 INTEGRATION
 * Author  : Amit Parihar
 ...
 *       • X and Y horizontal slipping eliminated! Feet stay
 *         planted in global space while the body tilts around them.
 *       • Gait algorithms are now completely mathematically isolated from 
 *         terrain angle logic.
 *   + Removed 2D linear imuZCompensation approximation.
 *   + Added physical body hip offset coordinates (hipX, hipY).
 *
 * ── WHAT'S NEW IN V3.4 (Summary) ─────────────────────────────
 *   + Mathematical 3D Matrix IK Engine replaces simple Z offsets.
 *   + Hexapod-style Terrain Sensing:
 *       • Legs actively extend downward when airborne and lock 
 *         instantly upon ground contact (`FOOT_LERP_AMT` physics).
 *   + Manual Overrides:
 *       • 'I' command toggles Auto-Levelling On/Off.
 *       • 'T' command toggles Tactile Feet Sensing On/Off.
 *       • '?' command shows updated status of both overrides.
 *
 * ── BUTTON MAP ───────────────────────────────────────────────
...
float bodyRoll    = 0.0f; // Integral controller — drives body to horizontal
float bodyPitch   = 0.0f; // Integral controller — drives body to horizontal
const float IMU_MAX_ANGLE = 20.0f; // Safety clamp for integral windup (degrees)
bool  imuReady  = false;
